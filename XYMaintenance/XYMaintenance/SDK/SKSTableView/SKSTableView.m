...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    self.SKSContentOffset = scrollView.contentOffset;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([correspondingIndexPath subRow] == 0)
    {
        ...
        if (expandableCell.isExpandable)
        {
            ...
            UIButton *expandableButton = (UIButton *)expandableCell.arrowButton;
            ...
            if (isExpanded)
            {
                expandableCell.arrowButton.transform = CGAffineTransformMakeRotation(M_PI);
        ...
        else
        {
            ...
            expandableCell.arrowButton = nil;
        }
...
//    [self didSelectAtIndexPath:indexPath tableView:self cell:cell];
    
//    if (self.lastIndexPath) {
//        SKSTableViewCell *cellL = (SKSTableViewCell *)[tableView cellForRowAtIndexPath:self.lastIndexPath];
//        [self didSelectAtIndexPath:self.lastIndexPath tableView:self cell:cellL];
//    }
//    self.lastIndexPath = indexPath;
}

- (void)didSelectAtIndexPath:(NSIndexPath *)indexPath tableView:(UITableView *)tableView cell:(SKSTableViewCell *)cell {
    if ([cell respondsToSelector:@selector(isExpandable)])
    {
        if (cell.isExpandable)
        {
...
- (void)setTableViewExpanded:(BOOL)isExpanded
{
    for (NSMutableDictionary *cellInfo in self.expandableCells[@0]) {
        [cellInfo setObject:@(isExpanded) forKey:kIsExpandedKey];
    }

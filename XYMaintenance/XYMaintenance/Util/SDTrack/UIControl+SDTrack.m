...
#import "UIControl+SDTrack.h"
#import "SDTrackTool.h"
#import <objc/runtime.h>
#import "HMSegmentedControl.h"

@implementation UIControl (SDTrack)
+(void)load {
...
- (void)trackAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    ...
    if ([self isKindOfClass:[HMSegmentedControl class]]&&[target class]) {
        HMSegmentedControl * segmentCtrl = (HMSegmentedControl *)self;
        NSString *eventID = [NSString stringWithFormat:@"CTRL_EVENT_%@_%ld", NSStringFromClass([target class]), (long)segmentCtrl.selectedIndex];
        if (eventID.length) {

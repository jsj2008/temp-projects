...
-(void)drawDebug:(NSArray*)linebreaks{
    ...
    if (debug) {
        ...
        if (debugShowCoordinates) {
            ...
            CGContextFillRect(ctx, CGRectMake(0, 0, [info sizeWithAttributes:@{NSFontAttributeName:displayFont}].width, debugHeight));
...
-(NSArray*)processTextLineToWords:(NSString*)textLine{
    ...
    for(NSInteger i= 0; i < [textLine length]; i ++)
    {
        [arr addObject:[textLine substringWithRange:NSMakeRange(i, 1)]];
    }

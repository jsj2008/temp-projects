...
{
    if ((!segment)||(!segment.buslines)||segment.buslines.count == 0)
    {
        return nil;
    }
    ...
    MAPolyline *busLinePolyline = [MANaviRoute polylineForBusLine:busline];
    if (busLinePolyline != nil)
    {
        ...
        NSMutableString* transitTitle = [[NSMutableString alloc]init];
        ...
        if (busline.departureStop!=nil)
        {
            [transitTitle appendString:[NSString stringWithFormat:@"(%@->%@)",busline.departureStop.name,busline.arrivalStop.name]];
        }

        bus.title = transitTitle;
                           ...
                           Polylines:(NSMutableArray *)polylines
{
    if (lastSegment)
    {
        ...
        if ((!lastSegment.buslines) || lastSegment.buslines.count==0) {
            return;
        }
        AMapBusLine* busline = lastSegment.buslines[0];
        MAPolyline *busLinePolyline = [self polylineForBusLine:busline];
        if (busLinePolyline != nil)
        {

...
- (void)setupUI
{
    ...
    _textField.textColor = XY_HEX(0x333333);
...
- (void)layoutSubviews
{
    ...
    if (_textField.text.integerValue == _minValue) {
        [_decreaseBtn setBackgroundImage:[UIImage imageNamed:@"jian_no"] forState:UIControlStateNormal];
    }
    if (_textField.text.integerValue == _maxValue) {
        [_increaseBtn setBackgroundImage:[UIImage imageNamed:@"jia_no"] forState:UIControlStateNormal];
    }
}
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([object isKindOfClass:[UITextField class]]) {
        if([keyPath isEqualToString:@"text"]) {
            int textFiledValue = [[change objectForKey:NSKeyValueChangeNewKey] intValue];
            if (textFiledValue == _minValue) {
                [_decreaseBtn setBackgroundImage:[UIImage imageNamed:@"jian_no"] forState:UIControlStateNormal];
            }else {
                [_decreaseBtn setBackgroundImage:[UIImage imageNamed:@"jian"] forState:UIControlStateNormal];
            }
            if (textFiledValue == _maxValue) {
                [_increaseBtn setBackgroundImage:[UIImage imageNamed:@"jia_no"] forState:UIControlStateNormal];
            }else {
                [_increaseBtn setBackgroundImage:[UIImage imageNamed:@"jia"] forState:UIControlStateNormal];
            }
        }
    }
}

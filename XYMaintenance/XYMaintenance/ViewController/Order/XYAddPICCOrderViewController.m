//
//  XYAddPICCOrderViewController.m
//  XYMaintenance
//
...
@implementation XYAddPICCOrderViewController
...
- (void)initializeUIElements{
    self.navigationItem.title = @"添加保险订单";
    ...
    self.uploadButton.uxy_acceptEventInterval = 2;
...
- (XYSelectDeviceTypeViewController*)deviceViewController{
    if (!_deviceViewController){
        ...
        _deviceViewController.customController.delegate = self;
    }
...
- (XYOrderDetailTopCell*)getTopCell{
    ...
    topCell.orderIdLabel.text = @"保险订单";
    topCell.statusLabel.text = @"";
    return topCell;
}
...
- (XYPICCPhotoCell*)photoCell{
    ...
    _photoCell.canRetakePhoto = true;
    return _photoCell;
}
...
- (void)doSelectDeviceType{
    self.deviceViewController.customController.delegate = self;
    self.deviceViewController.customController.preProductName = self.viewModel.orderDetail.product_name;
    self.deviceViewController.customController.preBrandName = self.viewModel.orderDetail.brand_name;
    self.deviceViewController.customController.preDeviceName = self.viewModel.orderDetail.mould_name;
    self.deviceViewController.customController.preDevicePrice = [NSString stringWithFormat:@"%@",@(self.viewModel.orderDetail.equip_price)];
...
- (void)goToRemarkView{
    ...
    remarkViewController.editable = true;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    TTDEBUGLOG(@"url = %@",info[UIImagePickerControllerReferenceURL]);
    //压缩保存图片
    ...
    if ([self.viewModel getPhotoSavingAvailable]) {//如果有保存权限
